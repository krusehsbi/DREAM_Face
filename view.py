...
def show_random_predictions(images, true_labels, save_path='predictions.png'):
    ...
    for i in range(len(images)):
        ...
        pred_face = float(ops.sigmoid(predictions[0][0]))  # Access the first element to get the scalar
        pred_age = float(ops.relu(predictions[1]))  # Access the first element for the scalar
        pred_gender = float(ops.argmax(predictions[2]))  # Argmax works directly as expected
